...
function initEventListeners() {
  ...
  document.addEventListener('click', (e) => {
    if (!categoryFilterToggle.contains(e.target) && !categoryFilterDropdown.contains(e.target)) {
...
// Update category filter dropdown with checkboxes
...
function updateCategoryLabel() {
  if (selectedCategories.length === 0) {
    ...
    categoryFilterLabel.textContent = selectedCategories[0].charAt(0).toUpperCase() + selectedCategories[0].slice(1);
  } else {
    categoryFilterLabel.textContent = `${selectedCategories.length} Selected`;
  }
}
